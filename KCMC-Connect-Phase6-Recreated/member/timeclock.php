...
<?php if (!$locked && !$open && $entries): ?><form method="post" style="margin-top:18px"><input type="hidden" name="csrf" value="<?=kcmc_h(kcmc_csrf())?>"><input type="hidden" name="action" value="submit_period"><label class="portal-check"><input type="checkbox" required><span>I reviewed this pay period and certify that the recorded time and work descriptions are complete.</span></label><button class="btn gold" type="submit">Submit Pay Period</button></form><?php endif; ?>
