...
if (kcmc_owner_configured()) { header('Location: ' . kcmc_url('admin/login.php')); exit; }
$cfg = kcmc_config();
...
if ($setupEnabled && $_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    if (!hash_equals($expected, $key)) $error = 'Invalid setup key.';
    ...
    elseif ($password !== $confirm) $error = 'Passwords do not match.';
        ...
        else { header('Location: ' . kcmc_url('admin/login.php?setup=1')); exit; }
    }
}
?><!doctype html><html><head><meta charset="utf-8"><meta name="viewport" content="width=device-width,initial-scale=1"><title>KCMC Owner Setup</title><link rel="stylesheet" href="<?=kcmc_h(kcmc_url('styles.css'))?>"><style>body{background:#0c1724;color:#fff}.adminbox{max-width:600px;margin:8vh auto;background:#fff;color:#142238;border-radius:22px;padding:30px}.adminbox input{width:100%;padding:13px;margin:7px 0 15px;border:1px solid #ccd3da;border-radius:10px}.adminbox button{padding:13px 18px;border:0;border-radius:999px;background:#17324c;color:#fff;font-weight:800}.error{color:#a02020}.notice{color:#4d3915}</style></head><body><main class="adminbox"><p class="eyebrow">KCMC CONNECT • OWNER SETUP</p><h1>Secure the publishing desk.</h1><?php if(!$setupEnabled): ?><p class="notice">Owner setup is disabled until a private setup key is configured on the server. This protects the publishing desk from public setup attempts.</p><?php else: ?><?php if($error): ?><p class="error"><?=kcmc_h($error)?></p><?php endif; ?><form method="post"><label>Setup key<input name="setup_key" required autocomplete="off"></label><label>New owner password<input name="password" type="password" required minlength="12"></label><label>Confirm password<input name="confirm" type="password" required minlength="12"></label><button>Create owner login</button></form><?php endif; ?><p><a href="<?=kcmc_h(kcmc_url())?>">← Back to KCMC Connect</a></p></main></body></html>
