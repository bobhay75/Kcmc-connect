...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    if (!kcmc_verify_csrf($_POST['csrf'] ?? null)) {
        ...
        if ($action === 'invite') {
            ...
            $targetEmail = kcmc_normalize_email((string)($_POST['invite_email'] ?? ''));
            if (!kcmc_valid_email($targetEmail)) {
                $error = 'That pending invitation could not be identified.';
            } else {
                $revokeResult = kcmc_update_json_store(KCMC_INVITES, ['version' => 1, 'invites' => []], function (array &$state) use ($targetEmail, $current): array {
                    return kcmc_revoke_pending_invites($state, $targetEmail, kcmc_role($current) === 'recovery_admin', time());
                });
                if (!empty($revokeResult['forbidden'])) {
                    $error = 'Only a recovery administrator can revoke a recovery-administrator invitation.';
                } elseif ((int)($revokeResult['revoked'] ?? 0) < 1) {
                    $error = 'No current pending invitation was found for that email.';
                } else {
                    kcmc_audit('member.invitation_revoked', ['count' => (int)$revokeResult['revoked']]);
                    $success = 'Pending invitation revoked for ' . $targetEmail . '.';
                }
            }
...
<?php if (isset($_GET['setup'])): ?><p class="portal-alert success">Recovery account created. Remove the server setup key now, then invite each Pastor administrator separately.</p><?php endif; ?>
...
<?php if ($pendingInvites): ?><section class="portal-card portal-section"><p class="eyebrow">PENDING INVITATIONS</p><h2>Awaiting activation</h2><p class="portal-fine">Only recipient and status details are shown here. Private invitation links and tokens are never displayed in this list.</p><div class="member-table"><div class="member-row header"><span>Name</span><span>Email</span><span>Role</span><span>Status</span><span>Expires</span><span>Action</span></div><?php foreach ($pendingInvites as $pending): ?><div class="member-row"><span><strong><?=kcmc_h((string)($pending['display_name'] ?? ''))?></strong></span><span><?=kcmc_h((string)($pending['email'] ?? ''))?></span><span><?=kcmc_h($roleNames[(string)($pending['role'] ?? '')] ?? (string)($pending['role'] ?? ''))?></span><span>Pending</span><span><?=kcmc_h(gmdate('M j, Y g:i A', strtotime((string)($pending['expires_at'] ?? ''))))?> UTC</span><span><form method="post"><input type="hidden" name="csrf" value="<?=kcmc_h(kcmc_csrf())?>"><input type="hidden" name="action" value="revoke_invite"><input type="hidden" name="invite_email" value="<?=kcmc_h((string)($pending['email'] ?? ''))?>"><button class="portal-link-button" type="submit">Revoke</button></form></span></div><?php endforeach; ?></div></section><?php endif; ?>
