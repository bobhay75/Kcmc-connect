    ...
    if (!kcmc_verify_csrf($_POST['csrf'] ?? null)) {
        ...
        if ($id === '' || !in_array($status, KCMC_INBOX_STATUSES, true)) {
            ...
            if (!empty($result['updated'])) {
                kcmc_audit('connection.follow_up_changed', ['previous_status'=>(string)$result['previous'],'status'=>(string)$result['current'],'count'=>1]);
...
foreach ($allRows as $row) { $kind=(string)($row['kind']??''); if(isset($typeCounts[$kind])) $typeCounts[$kind]++; }
...
$exportUrl = kcmc_url('admin/connections-export.php?status=' . rawurlencode($filter));
?><!doctype html><html lang="en"><head><meta charset="utf-8"><meta name="viewport" content="width=device-width,initial-scale=1"><meta name="robots" content="noindex,nofollow"><title>Connection Inbox • KCMC Connect</title><link rel="stylesheet" href="<?=kcmc_h(kcmc_url('styles.css'))?>"><style>
...
@media(max-width:760px){.shell{width:min(100% - 18px,1240px);margin-top:16px}.card{padding:12px;background:transparent;border:0}.summary{grid-template-columns:repeat(2,minmax(0,1fr))}.export-link{margin-left:0}.table{overflow:visible}.row.head{display:none}.row{display:block;min-width:0;background:#fff;border:1px solid #dce3e7;border-radius:14px;padding:10px 14px;margin:12px 0;box-shadow:0 2px 8px rgba(23,50,76,.05)}.row>span{display:grid;grid-template-columns:104px minmax(0,1fr);gap:10px;padding:8px 0;border-top:1px solid #edf0f2;overflow-wrap:anywhere}.row>span:first-child{border-top:0}.row>span::before{font-weight:800;color:#526575}.row>span:nth-child(1)::before{content:'Type'}.row>span:nth-child(2)::before{content:'Name'}.row>span:nth-child(3)::before{content:'Email'}.row>span:nth-child(4)::before{content:'Phone'}.row>span:nth-child(5)::before{content:'Interest'}.row>span:nth-child(6)::before{content:'Note'}.row>span:nth-child(7)::before{content:'Received'}.row>span:nth-child(8)::before{content:'Follow-up'}.status-form{flex-wrap:wrap}.status-form select{max-width:none;flex:1 1 120px}.status-form button{min-height:40px}.filters{align-items:stretch}.filters a{display:inline-flex;align-items:center;justify-content:center}.note{white-space:pre-wrap}}
</style></head><body><main class="shell"><nav class="nav" aria-label="Administrator navigation"><a href="<?=kcmc_h(kcmc_url('admin/'))?>">← Publishing Desk</a><a href="<?=kcmc_h(kcmc_url('admin/rsvps.php'))?>">Event RSVPs</a></nav><p class="eyebrow">KCMC CONNECTIONS</p><h1>Connection inbox</h1><p class="muted">Newest 300 visit, volunteer and group-interest requests. Contact details and notes remain inside the authenticated administrator area.</p><?php if($error):?><p class="portal-alert error" role="alert"><?=kcmc_h($error)?></p><?php endif;?><?php if($success):?><p class="portal-alert success" role="status"><?=kcmc_h($success)?></p><?php endif;?>
<section class="summary" aria-label="Connection request totals"><?php foreach($typeCounts as $kind=>$count):?><article><strong><?=kcmc_h((string)$count)?></strong><span><?=kcmc_h($labels[$kind])?></span></article><?php endforeach;?><article><strong><?=kcmc_h((string)$statusCounts['new'])?></strong><span>New</span></article><article><strong><?=kcmc_h((string)$statusCounts['contacted'])?></strong><span>Contacted</span></article><article><strong><?=kcmc_h((string)$statusCounts['closed'])?></strong><span>Closed</span></article></section>
<nav class="filters" aria-label="Follow-up status filters"><span class="muted">Show:</span><?php foreach(['all'=>'All','new'=>'New','contacted'=>'Contacted','closed'=>'Closed'] as $value=>$label):?><a href="<?=kcmc_h(kcmc_url('admin/connections.php?status='.rawurlencode($value)))?>" <?=$filter===$value?'aria-current="page"':''?>><?=kcmc_h($label)?></a><?php endforeach;?><a class="export-link" href="<?=kcmc_h($exportUrl)?>">Download this view as CSV</a></nav>
<section class="card"><?php if(!$rows):?><p>No connection requests match this view.</p><?php else:?><div class="table"><div class="row head"><span>Type</span><span>Name</span><span>Email</span><span>Phone</span><span>Service / Interest</span><span>Note</span><span>Received</span><span>Follow-up</span></div><?php foreach($rows as $row):$kind=(string)($row['kind']??'');$detail=trim((string)($row['service']??''))?:trim((string)($row['interest']??''));$rowStatus=kcmc_inbox_status($row['status']??null);?><div class="row"><span><?=kcmc_h($labels[$kind]??ucfirst($kind))?></span><span><strong><?=kcmc_h((string)($row['name']??''))?></strong></span><span><a href="mailto:<?=kcmc_h((string)($row['email']??''))?>"><?=kcmc_h((string)($row['email']??''))?></a></span><span><?php $phone=trim((string)($row['phone']??''));if($phone!==''):?><a href="tel:<?=kcmc_h(preg_replace('/[^0-9+]/','',$phone)??'')?>"><?=kcmc_h($phone)?></a><?php else:?>—<?php endif;?></span><span><?=kcmc_h($detail)?></span><span class="note"><?=kcmc_h((string)($row['message']??''))?></span><span><?=kcmc_h(kcmc_local_date_value((string)($row['submitted_at']??''),'M j, Y g:i A'))?></span><span><form class="status-form" method="post"><input type="hidden" name="csrf" value="<?=kcmc_h(kcmc_csrf())?>"><input type="hidden" name="id" value="<?=kcmc_h((string)($row['id']??''))?>"><select name="status" aria-label="Follow-up status for <?=kcmc_h((string)($row['name']??'request'))?>"><?php foreach(KCMC_INBOX_STATUSES as $status):?><option value="<?=kcmc_h($status)?>" <?=$rowStatus===$status?'selected':''?>><?=kcmc_h(kcmc_inbox_status_label($status))?></option><?php endforeach;?></select><button type="submit">Save</button></form></span></div><?php endforeach;?></div><?php endif;?></section></main></body></html>
